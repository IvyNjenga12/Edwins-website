...
function Question() {
  ...
  const trends = ['Why is my girl so fine?', 'Why does Ivy love you?', `When will you marry Ivy??????`,]

  const handleSearch = (e) => {
    e.preventDefault()
    if (searchQuery.toLowerCase() === 'how long have we known each other' || searchQuery.toLowerCase() === 'how long have we known each other') {
          navigate("/timer");
       ...
       <div
        ...
        <div className="flex flex-col min-h-screen w-full max-w-[480px] mx-auto ">
          ...
          <div className='mt-10  px-5 text-white/50'>
            <p>Pro tip: try searching "How long have we kown each other" ;)</p>
          </div>
        </div>
      )}
    </div>
